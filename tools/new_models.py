...
#                 last = h
#           layers.append(nn.Linear(last, 2))  # binary classification -> logits for 2 classes
...
def create_model_pipeline(model_name,best_params,numerical_features,categorical_features):
    ...
    if model_name == 'RF': #randomforest
        ...
        model = XGBClassifier()
    # elif model_name == 'NN_gradient': 
        # model = SimpleMLP(input_dim=len(numerical_features)+len(categorical_features)) # Example input dimension
        # SimpleNet(input_dim=len(numerical_features)+len(categorical_features)) # Example input dimension
    else:
